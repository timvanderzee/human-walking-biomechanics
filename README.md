...
There are two versions of MATLAB data:
...
#### 5 strides data ####
There are 9 5 stride files since there is no 5 stride file for subject 10 (see Missing/Excluded Data). For example, for subject 2 the 5 stride 
...
*Location*: These variables are stored in the field called ‘Participant’

*Variables*: 

•	**Height**- Participant’s height in metres

•	**Mass**- Participant’s mass in kilograms

•	**centerofmass**- Participant’s centre of mass in metres

Example: **Data.Participant.Mass** contains the mass of the participant.
...
### **All Strides Data & 5 Strides Data** ###
1.	Participant 6 trial 31, the raw data will have variables full of zeros for the motion capture variables, and empty variables for all variables relying on motion capture data as there was no motion capture data collected for this trial.
2.  Participant 6 trial 21, only includes 4 strides and is therefore excluded.
3.	Participant 7 trial 24 doesn’t exist
4.	Participant 9 does not have upper body motion capture data for any of the trials. The following markers were not used for Participant 9: LAC, RAC, LEP, REP, LWR, RWR. Any variables related to these variables are empty in the raw data.

### **5 Strides Data only** ###
1.	For all participant, trials 26-30 are not included in the 5 strides data. These trials are the step width trials and since the heel strikes are difficult to identify, defining steps and picking a 5 stride range was not complete.
2.	Participant 3 trials 4 and subject 9 trial 14 have an apparent sync error.
3.	Participant 4 trial 1 has incorrect stepping on the force platform (like trial 26-30).
4.	Participant 10, all trials. Participant 10 appears to have small pelvis measurements, this is possibly a result of incorrect marker placement. Due to this error, we decided to exclude this participant from 5 strides files.
